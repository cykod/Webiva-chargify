...
class Chargify::AdminController < ModuleController
  ...
  def options
    ...
    @pro_handler_options = Chargify::ProSubscriptionHandler.handler_options params[:pro_handler_options]

    if request.post? && @handler_options.valid? && @options.valid? && @pro_handler_options.valid?
